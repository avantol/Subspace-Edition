...
// for ANY MCForm: "E? F!<code>" (relayed: "E? F!<code> *DE* ORIGIN").
// A submission carries the form code without "E?" and gets its ACK.
bool UI_Constructor::isFormPull(QString const &text) {
    static QRegularExpression const re(
        QStringLiteral("\\bE\\?\\s+F![A-Z0-9]+"));

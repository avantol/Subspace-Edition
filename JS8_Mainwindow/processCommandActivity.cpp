    ...
    while (!m_rxCommandQueue.isEmpty()) {
        ...
        // its own frame addressed to us.
            ...
            if (match.hasMatch() && !isGroupCall) {
                ...
                // [operator 2026-09-14] PULLS only, and any form once
                // the MAGNET special case is known (m_superSpotterSeen);
                // a 701A/B/C pull always, since it IS the signature.
                // A relayed form SUBMISSION keeps the ACK: it is the
                // sender's only delivery confirmation (the 480 gate on
                // any F!701x text withheld it for submissions too).
                if (isFormPull(d.text) &&
                    (m_superSpotterSeen || isMagnetFormText(d.text))) {
                    reply.clear();
                    qCWarning(mainwindow_js8)
                        << "[ACKQUIET #222] relay ACK withheld: payload"
                        << "is a form PULL, which answers itself"
        ...
        // PROCESS MSG
            ...
            // use it for the ack if there is one
            //
            // [ackquiet #222] A MSG is ALWAYS acknowledged, @MAGNET 701
            // forms included. Build 481 withheld the ACK for a form
            // arriving as MSG; reverted on the SuperSpotter developer's
            // word (Felix KG5RKW via operator, 2026-09-12): the MSG ACK
            // is the sender's only delivery confirmation and is wanted.
            // The #222 gate applies to the relay block's free-text ACK
            // only.
            reply = QString("%1 ACK").arg(calls.length() > 1 ? d.relayPath
                                                             : d.from);
